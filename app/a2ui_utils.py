...
def _extract_text_fallback(messages: list[dict]) -> str:
    """Extract human-readable text from A2UI components as a safety fallback."""
    lines = []
    for m in messages:
        ...
        for c in surface.get("components") or []:
            ...
            if isinstance(comp, dict) and "Text" in comp:
                t = comp["Text"].get("text", {})
                literal = t.get("literalString") if isinstance(t, dict) else None
                if literal:
                    lines.append(literal)
    return "\n".join(lines) if lines else "Here are your recipe results."


def _sanitize_image_components(messages: list[dict]) -> None:
    """Ensure Image components have a valid http(s) URL in literalString format."""
    for m in messages:
        surface = m.get("surfaceUpdate")
        if not isinstance(surface, dict):
            continue
        for c in surface.get("components") or []:
            ...
            if isinstance(url, str):
                ...
                raw_url = url.get("literalString") or url.get("url")

            if isinstance(raw_url, str) and _HTTP_URL_RE.match(raw_url):
                comp["Image"] = {"url": {"literalString": raw_url}}
                continue
...
def _surface_is_renderable(messages: list[dict]) -> bool:
    ...
    In adk web, the root component MUST be a Card. If root is a Column or missing,
    adk web's web component renders blank.
    """
    all_ids: set = set()
    all_refs: set = set()
    roots: list = []
    ...
    # CRITICAL adk web check: Root component MUST be a Card!
    for root in roots:
        root_comp = comp_map.get(root)
        if not isinstance(root_comp, dict) or "Card" not in root_comp:
            return False
...
def a2ui_callback(
    ...
    """Convert A2UI JSON in text output to rendered components, with text fallback for safety."""
    ...
    for part in llm_response.content.parts:
        ...
        fallback_text = _extract_text_fallback(messages)

        if not _surface_is_renderable(messages):
            # If surface is invalid or root is not Card, return extracted text so it's NEVER blank!
            return LlmResponse(
        ...
        new_parts = [
            types.Part(text=fallback_text),
            *[_wrap_a2ui_part(m) for m in messages],
        ]
        return LlmResponse(
